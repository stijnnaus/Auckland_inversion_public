...
I've included a test script (run_scripts/run_test.py) that should work when the environment is set up correctly, and this also describes very roughly how the code should be used.

If you plan in any way to use this code it is highly adviced to contact Stijn Naus (stijn.naus@wur.nl) or Daemon Kennett (daemon.kennett@niwa.co.nz). An attempt has been made to structure and comment the code, but it is a work in progress, and the system is developed specifically for Auckland, New Zealand.
